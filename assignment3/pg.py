...
class PG(object):
  ...
  def build_policy_network_op(self, scope="policy_network"):
    ...
    if self.discrete:
      ...
      self.sampled_action = tf.squeeze(tf.multinomial(action_logits, 1, output_dtype=tf.int32), axis=-1)
      ...
      self.sampled_action = action_means + tf.exp(log_std) * tf.random.normal(shape=tf.shape(action_means))

      scale = tf.linalg.diag(tf.exp(log_std))
      mnd = tf.contrib.distributions.MultivariateNormalDiag(action_means, scale)
      self.logprob = tf.log(mnd.prob(self.action_placeholder))
    #######################################################
    #########          END YOUR CODE.          ############
  ...
  def add_baseline_op(self, scope="baseline"):
    ...
    self.baseline = tf.squeeze(build_mlp(self.observation_placeholder, 1, scope, self.config.n_layers,
                      self.config.layer_size, self.config.activation), axis=-1)
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      returns = [sum([(gamma ** (t - t0)) * rewards[t] for t in range(t0, T)]) for t0 in range(T)]
      #######################################################
      #########          END YOUR CODE.          ############
