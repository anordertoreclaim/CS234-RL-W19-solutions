...
class Linear(DQN):
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            state_flattened = tf.layers.Flatten()(state)
            out = tf.layers.Dense(num_actions)(state_flattened)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        operations = []
        tgt_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=target_q_scope)
        src_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=q_scope)

        for tgt_var, src_var in zip(tgt_vars, src_vars):
    ...
    def add_loss_op(self, q, target_q):
        ...
        max_q_target = tf.reduce_max(target_q, axis=1)
        discounted_reward = self.config.gamma * tf.where(self.done_mask, tf.zeros_like(max_q_target), max_q_target)
        q_samp = self.r + discounted_reward

        q_sa = tf.diag_part(tf.matmul(q, tf.one_hot(self.a, num_actions, axis=0)))
        loss = tf.reduce_mean(tf.squared_difference(q_samp, q_sa))
        self.loss = loss
    ...
    def add_optimizer_op(self, scope):
        ...
        self.grad_norm = tf.global_norm([grads_and_vars[i][0] for i in range(len(grads_and_vars))])
